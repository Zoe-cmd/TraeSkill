...
| 阶段 | 角色 | Skill 文件 | 核心产出 |
|------|------|-----------|----------|
| Phase 0 | 项目经理 | `skills/project-manager.md` | 项目计划、任务清单、决策日志 |
| Phase 0 | 技术负责人 | `skills/tech-lead.md` | 技术选型决策 |
| Phase 1 | 产品经理 | `skills/product-manager.md` | PRD 产品需求文档 |
| Phase 1 | UI/UX 设计师 | `skills/ui-ux-designer.md` | 设计系统、用户流程、线框图 |
| Phase 1 | 系统架构师 | `skills/solution-architect.md` | 架构设计文档 |
| Phase 2 | 数据库工程师 | `skills/database-engineer.md` | 数据库 Schema、迁移计划 |
| Phase 2 | 后端工程师 | `skills/backend-engineer.md` | API 文档、后端代码 |
| Phase 3 | 前端工程师 | `skills/frontend-engineer.md` | 前端页面、组件 |
| Phase 3 | AI 工程师 | `skills/ai-engineer.md` | AI 功能、RAG 管线 |
| Phase 4 | 测试工程师 | `skills/qa-engineer.md` | 测试计划、测试报告、缺陷报告 |
| Phase 4 | 安全工程师 | `skills/security-engineer.md` | 安全审计报告 |
| Phase 5 | 代码评审工程师 | `skills/code-reviewer.md` | 代码审查报告、重构建议 |
| Phase 5 | 运维工程师 | `skills/devops-engineer.md` | 部署计划、CI/CD 配置 |

### 缺陷修复回路

当测试工程师发现 Critical/High Bug 时，触发特殊流程：

```
QA 发现 Critical/High Bug
    ↓
QA 生成 docs/交接/缺陷修复交接-{BUG-ID}.md（含完整 Bug 详情）
    ↓
QA 根据 Bug 涉及的模块判断修复负责人
    ↓
修复负责人读取交接文档 → 修复 Bug → 更新状态
    ↓
QA 执行回归验证
    ↓
回归通过 → Bug 关闭，继续正常流程
...
在 Cursor Chat 中用 `@file:skills/角色名.md` 引用。每个角色一个新对话。

### GitHub Copilot / Codex

```bash
# 在 Copilot Chat 中（VS Code）
请阅读 #file:skills/product-manager.md
...
```bash
# 将 Skill 文件作为系统指令
gemini --system-instruction "$(cat skills/product-manager.md)"

# 或在对话中引用
gemini
> 请阅读 skills/product-manager.md，你是产品经理，分析需求：[描述]
```
...
1. **选择角色**：根据当前阶段，选择 `skills/角色名.md`
