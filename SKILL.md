...
当 QA 在 Phase 4 发现 Critical/High Bug 时，**不得直接交接给下游角色**。必须触发缺陷修复回路：
...
QA 写入 docs/bug-report.md（含完整详情）
    ↓
QA 生成专项交接文档 docs/handoff-bugfix-{BUG-ID}.md
    ↓
QA 根据 Bug 涉及模块判断修复负责人：
  ├── 后端代码问题 → 激活后端工程师（缺陷修复模式）
  ├── 前端代码问题 → 激活前端工程师（缺陷修复模式）
  ├── AI 功能问题 → 激活AI工程师（缺陷修复模式）
  ├── 数据库问题 → 激活数据库工程师（缺陷修复模式）
  └── 架构问题 → 激活系统架构师
    ↓
修复负责人读取交接文档，修复 Bug
    ↓
...
**关键规则**：交接文档中必须包含完整的 Bug 详情（重现步骤、实际结果、预期结果、涉及文件、修复建议），**不得仅引用 Bug 编号**。修复负责人通过读取交接文档即可获得全部上下文，无需额外询问。
